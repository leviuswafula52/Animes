...
//header
const animeQuote = document.createElement("p");
const parentEl = document.querySelector(".background-contents");
animeQuote.innerHTML =
  "<span class='line2'>“Those who stand at the top determine what’s wrong and what’s right! This very place is neutral ground!</span> Justice will prevail, you say? But, of course, it will! Whoever wins this war becomes justice!” ";
animeQuote.classList.add("quote");
parentEl.appendChild(animeQuote);
...
const a2 = annotate(animeQuote, {
  type: "underline",
  color: "yellow",
  strokeWidth: 0.9,
});
const a3 = annotate(subScription, {
  type: "crossed-off",
  color: "red",
  strokeWidth: 3,
  iterations: 1,
  ...
  .then((response) => {
    ...
    datas.map((data) => {
      ...
      sorted_animes.map((sorted_anime) => {
        const synopsis = truncateDesc(
          sorted_anime.synopsis,
          100,
          sorted_anime.link
        );
      ...
      textElement.addEventListener("click", () => {
        ...
        datas.map((animesGerne) => {
          ...
          if (animeGerneValue.includes(text)) {
            let animeGernes = [];
            animeGernes.push(animesGerne);

            animeGernes.map((animeGernesOutput) => {
              const synopsis = truncateDesc(
                animeGernesOutput.synopsis,
                100,
                animeGernesOutput.link
              );
